...
重要な点：
1. あなたは一度に様々な観点を考慮するのが苦手なので、お題に関連した用語の洗い出しや、一度作成した回答の評価などのタスクはすべて切り出して、サブタスクとして並列処理するようにしてください。

2. 評価は以下の観点ごとに独立したタスクとして実行してください：
   - 意外性（予想を裏切る面白さがあるか）
   - 端的さ（説明的すぎず、短くまとまっているか。理想は20文字以内）
   - 共感性（「あるある」と思えるか）
   - 機知性（頭の良さを感じるか）
   - 理不尽さ（「ありそうでありえない」感じか）

3. 単純に嫌な特徴を述べるだけでは面白くありません。意外性や理不尽さが重要です。

4. あとからどんな検討をしたのかがわかるようなレポートをステップごとに更新していくようにしてください。
